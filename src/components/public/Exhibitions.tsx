...
import { getExhibitions, Exhibition } from '@/services/exhibitionsService';
...
export default function Exhibitions() {
  ...
  const goToPrevious = () => {
    if (selectedImage !== null) {
      setSelectedImage((selectedImage - 1 + exhibitions.length) % exhibitions.length);
    }
  };

  const goToNext = () => {
    if (selectedImage !== null) {
      setSelectedImage((selectedImage + 1) % exhibitions.length);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {loading ? (
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-gray-400 text-lg">Loading exhibitions...</div>
          </div>
        ) : exhibitions.length === 0 ? (
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-gray-400 text-lg">No exhibitions available yet.</div>
          </div>
        ) : (
        ...
        {selectedImage !== null && (
          ...
          >
            ...
            <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white text-sm">
              {selectedImage + 1} / {exhibitions.length}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
